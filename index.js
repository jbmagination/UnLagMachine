...
async function bindEvents(bot) {
    ...
    bot.on('spawn', () => {
        bot.chat("Hello, world! I'm online. Made by JBMagination#5512 - type ${config.prefix}help for a list of commands");
        setTimeout(() => { introMessageSent = true; }, 1)
    });

    bot.on('playerJoined', (player) => {
        if (player.username == "uptime_check") bot.chat(`Uptime check: I'm active! Type ${config.prefix}help for a list of commands`);
    });

    async function commands(username, message, whisper) {
        if (message.startsWith(`${config.prefix} `)) return;
        else if (knownBots.includes(username) && (!(whitelistedBots.includes(username)))) return;
        else if (blockedUsers.includes(username)) return;
        else {
            if ((message.startsWith(`${config.prefix}`))) { message = message.substring(config.prefix.length); console.log(message); }
            console.log(message)
            ...
            //#region | +hashcheck
                ...
                if (args[0]) {
                    ...
                    else if (args[0].toUpperCase() == (("099FDEAF848A3286FF6EBDB082283571F8DC72B0F3F692A666940BB3C2B4F371") || ("C7D7E3CDD244865B043519E47AE0D923E4C8BD8DDB8934E110B5C0E3E8006774"))) {
                        console.log(`${username} scanned the old IP hash`)
                        bot.whisper(username, "This hash matches the *old* IP! If this hash is of the IP you're connected with, then you may be connected through a honeypot.")
                    }
                    else if (args[0].toUpperCase() == (("12CA17B49AF2289436F303E0166030A21E525D266E209267433801A8FD4071A0") || ("723EC3738011F5417ABC2CFCD0AB6DA6ECC67E97918FC189EE4E5C98152CEA2A"))) {
                        console.log(`${username} scanned the loopback hash`)
                        bot.whisper(username, "This hash is a loopback! You're definitely not connecting through this IP (...probably)")
                    }
                    else if (honeypotHashes.includes(args[0].toUpperCase())) {
                        console.log(`${username} scanned a known honeypot IP hash`)
                        bot.whisper(username, "This hash matches a known honeypot IP. You may experience frequent disconnects and timeouts, although you will still be able to play on the server. Your public IP will be made known to the honeypot (although this applies to the real server too!)")
                    }
                    else {
                        console.log(`${username} scanned an unknown hash - ${args[0].toUpperCase()}`)
                        bot.whisper(username, "This hash is not known, but if this hash is of the IP you're connected with, then it's almost guaranteed that you're connected through a honeypot.")
                        bot.whisper(username, "You may experience frequent disconnects and timeouts, although you will still be able to play on the server. Your public IP will be made known to the honeypot (although this applies to the real server too!)")
                    }
                }
                else bot.whisper(username, "Put the IP you're connecting to this server with in https://coding.tools/sha256 - then paste the \"hash\" it gives back to you as an argument for this command: ${config.prefix}hashcheck <hash>")
            }
            //#endregion
            ...
            //#region | +online
                ...
                bot.whisper(username, `${playerCount} ${players} (excluding ${botCount} ${bots}, ${Object.keys(bot.players).length} members total): ${playerList.join(', ')}`)
            }
            //#endregion

            else if (whisper == true) bot.whisper(username, `Hi, ${username}! I'm a bot. Whisper me \"${config.prefix}help\" or \"help\" to get a list of commands.`)
        }
    }
